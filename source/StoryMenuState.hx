	...
	function selectWeek() {
		if (isWeekUnlocked(weeks[curWeek].id)) {
			...
			if (FileSystem.exists(Paths.instNoLib(PlayState.storyPlaylist[0].toLowerCase()))) {
				PlayState.SONG = Song.loadFromJson(PlayState.storyPlaylist[0].toLowerCase() + CoolUtil.getDiffSuffix(curDifficulty), PlayState.storyPlaylist[0].toLowerCase());
			} else {
				PlayState.SONG = Song.PEloadFromJson(PlayState.storyPlaylist[0].toLowerCase() + CoolUtil.getDiffSuffix(curDifficulty), PlayState.storyPlaylist[0].toLowerCase());
			}
