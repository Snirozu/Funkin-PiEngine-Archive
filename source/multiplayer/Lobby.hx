...
class ClientUpdate extends FlxBasic {
	override function update(elapsed) {
		super.update(elapsed);

		if (Lobby.isHost) {
			if (Lobby.client != null) {
				if (!Lobby.client.client.isConnected()) {
					Lobby.client.client.disconnect();
					Lobby.client.client = null;
                    return;
				}

                try {
				    Lobby.client.client.update();
                } catch (exc) {
...
class Lobby extends MusicBeatState {
    ...
    function addGeneralUI():Void {
        ...
        songsDropDown = new UIDropDownMenu(10, 20, CoolUtil.getSongs(), function(song:String, i) {
			...
			difficultyDropDown.strList = CoolUtil.difficultyList(Lobby.curSong, true);
    ...
    function goToSong(song:String, diff:Int) {
        ...
        if (FileSystem.exists(Paths.instNoLib(song))) {
			PlayState.SONG = Song.loadFromJson(song + CoolUtil.getDiffSuffix(difficultyDropDown.selectedLabel), song);
		}
        else if (FileSystem.exists(Paths.PEinst(song))) {
			PlayState.SONG = Song.PEloadFromJson(song + CoolUtil.getDiffSuffix(difficultyDropDown.selectedLabel), song);
		}
